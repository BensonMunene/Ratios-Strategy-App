...
# STRATEGY 1
# ==============================================
def backtest_strategy_1(df, asset="YMAX", initial_investment=10_000, corr_window=14):
    """
...
# ==============================================
# STRATEGY 2 (Revised to accept dynamic thresholds)
# ==============================================
def backtest_strategy_2(
    ...
    """
    Strategy 2 with dynamic thresholds:
...
# STRATEGY 3
# ==============================================
def backtest_strategy_3(df, asset="YMAX", initial_investment=10_000, vix_threshold=20, vvix_threshold=95):
    """
    Enter if VIX < vix_threshold and VVIX < vvix_threshold
    Exit if VIX > vix_threshold or VVIX > vvix_threshold + 5
    (Example extension: user sets VIX=20, VVIX=95 => exit if VIX>20 or VVIX>100)
    """
    temp_df = df.copy()
    temp_df.sort_values("Date", inplace=True)
    temp_df.reset_index(drop=True, inplace=True)
    ...
    def in_market_condition(vix, vvix):
        return (vix < vix_threshold) and (vvix < vvix_threshold)

    def exit_condition(vix, vvix):
        return (vix > vix_threshold) or (vvix > vvix_threshold + 5)
...
# ==============================================
# New: Combined Strategies Backtest Function
# ==============================================
def backtest_combined_strategies(
    df,
    priorities,
    asset="YMAX",
    initial_investment=10_000,
    ...
    """
    Combined strategy logic (simplified):
      - For each chosen strategy, we run the backtest to get an "In_Market" signal.
      - We check them in the user-specified priority order:
         If priority 1 is "in market" on a given day, we pick that strategy.
         Else check priority 2, etc.
      - If none are "in market", we do "No Investment".
      - We then grow the portfolio by +0.1% each day we're in market (toy logic).
      - Return a DataFrame with columns: Date, Portfolio_Value, Strategy, In_Market, etc.
    """
    # Sort data, reset index
    df = df.sort_values("Date").reset_index(drop=True)
    combined_df = df.copy()
    combined_df["Portfolio_Value"] = initial_investment
    combined_df["Active_Strategy"] = "No Investment"
    combined_df["In_Market"] = False

    # For each strategy in priorities, run the normal backtest to get signals
    # We'll store them in a dictionary
    def get_signal_column(strategy_name):
        """Return a Series of booleans: True if 'in market', else False."""
        if strategy_name == "Strategy 1":
            res = backtest_strategy_1(df, asset=asset, initial_investment=initial_investment, corr_window=corr_window)
            # "No Investment" => not in market
            return res["Strategy"].apply(lambda x: x != "No Investment")
        elif strategy_name == "Strategy 2":
            res = backtest_strategy_2(
                df,
                asset=asset,
    ...
    # Create a column for each strategy's signal
    for i, strat_name in enumerate(priorities):
        combined_df[f"Signal_{strat_name}"] = get_signal_column(strat_name)

    # Determine active strategy per day
    def pick_active_strategy(row):
        for strat_name in priorities:
            if row[f"Signal_{strat_name}"] == True:
                return strat_name
        return "No Investment"

    combined_df["Active_Strategy"] = combined_df.apply(pick_active_strategy, axis=1)
    combined_df["In_Market"] = combined_df["Active_Strategy"] != "No Investment"

    # Simplified daily portfolio update: +0.1% if in market, else no growth
    for i in range(1, len(combined_df)):
        prev_val = combined_df.loc[i-1, "Portfolio_Value"]
        if combined_df.loc[i, "In_Market"]:
            combined_df.loc[i, "Portfolio_Value"] = prev_val * 1.001  # +0.1% daily
        else:
            combined_df.loc[i, "Portfolio_Value"] = prev_val

    # For plotting and distribution, rename "Active_Strategy" -> "Strategy"
    combined_df["Strategy"] = combined_df["Active_Strategy"]
    # For a daily return
...
def calculate_performance_metrics(portfolio_df):
    ...
    if "Date" in df.columns:
        ...
        # If there's no "Date" column, skip
        num_days = 365
...
def plot_strategy_distribution(df):
    if "Strategy" not in df.columns:
        return px.bar(title="No Strategy Column Found")
...
def plot_entry_exit(df, asset_name="YMAX"):
    ...
    # If no "Strategy" column, just skip
    if "Strategy" not in temp_df.columns:
        temp_df["Strategy"] = "No Investment"

    if "In_Market" not in temp_df.columns:
        # If "Strategy" != "No Investment" => In_Market
        temp_df["In_Market"] = temp_df["Strategy"].apply(lambda x: x != "No Investment")
    ...
    # If there's no "Portfolio_Value", skip
    if "Portfolio_Value" in temp_df.columns:
        fig.add_trace(
            go.Scatter(
                x=temp_df["Date"],
...
""")
    ...
    # Three columns: Asset Selection, Date Range, Strategy Selection
    ...
    with col_sel2:
        ...
        # Now includes a fourth option for combining strategies.
        strategy_choice = st.radio(
            "Select strategy to backtest:",
            options=["Strategy 1", "Strategy 2", "Strategy 3", "Combine Strategies"]
        )

    st.markdown("---")

    # ============= SHOW PRIORITY UI IF "Combine Strategies" SELECTED ============
    priority_strats = []
    num_strats = 0
    if strategy_choice == "Combine Strategies":
        st.subheader("Combine Strategies Parameters")
        num_strats = st.selectbox("Number of strategies to combine", [2, 3])
        for i in range(num_strats):
            strat = st.selectbox(
                f"Priority {i+1} Strategy",
                options=["Strategy 1", "Strategy 2", "Strategy 3"],
                key=f"priority_{i}"
            )
            priority_strats.append(strat)

    # ============= DETERMINE WHICH STRATEGY WINDOWS TO SHOW ============
    # We show param windows if:
    # 1) The user individually selected that strategy, OR
    # 2) The user selected Combine Strategies AND included that strategy in priority_strats.

    def is_in_combine(strat_name):
        return (strategy_choice == "Combine Strategies") and (strat_name in priority_strats)

    show_strat1_params = (strategy_choice == "Strategy 1") or is_in_combine("Strategy 1")
    show_strat2_params = (strategy_choice == "Strategy 2") or is_in_combine("Strategy 2")
    show_strat3_params = (strategy_choice == "Strategy 3") or is_in_combine("Strategy 3")

    # ============= STRATEGY 1 PARAMS ============
    if show_strat1_params:
        st.subheader("Parameters for Strategy 1")
        corr_window = st.slider("Select correlation window (days) for Strategy 1:", 1, 30, 14, key="corr_window_strat1")
    else:
        corr_window = 14  # fallback

    # ============= STRATEGY 2 PARAMS ============
    if show_strat2_params:
        st.subheader("Parameters for Strategy 2")
        vix_lower = st.slider("VIX Lower", 0, 50, 15, 1, key="vix_lower_strat2")
        vix_upper = st.slider("VIX Upper", 0, 50, 20, 1, key="vix_upper_strat2")
        vvix_lower = st.slider("VVIX Lower", 0, 200, 90, 1, key="vvix_lower_strat2")
        vvix_upper = st.slider("VVIX Upper", 0, 200, 100, 1, key="vvix_upper_strat2")
        vvix_reentry_upper = st.slider("VVIX Re-Entry Upper", 0, 200, 95, 1, key="vvix_reentry_strat2")
    else:
        vix_lower = 15
        vix_upper = 20
        vvix_lower = 90
    ...
    # ============= STRATEGY 3 PARAMS ============
    if show_strat3_params:
        st.subheader("Parameters for Strategy 3")
        vix_threshold = st.slider("Select VIX threshold (Strategy 3):", 1, 40, 20, key="vix_threshold_strat3")
        vvix_threshold = st.slider("Select VVIX threshold (Strategy 3):", 1, 120, 95, key="vvix_threshold_strat3")
    else:
        vix_threshold = 20
        vvix_threshold = 95
    ...
    if run_backtest:
        ...
        # 3) Compute rolling correlations using Strategy 1's corr_window
        #    (We only have a single corr_window from Strategy 1 if user is combining strategies,
        #     or default=14 if Strategy 1 not used at all.)
        ps_df = compute_rolling_correlations(all_assets, corr_window)
        ps_df.reset_index(inplace=True)
        prices_and_stats_df = ps_df.copy()

        # 4) Backtest function
        def process_asset(asset_name, chosen):
            if chosen == "Strategy 1":
                return backtest_strategy_1(ps_df, asset=asset_name, corr_window=corr_window)
            elif chosen == "Strategy 2":
            ...
            elif chosen == "Strategy 3":
                return backtest_strategy_3(
                    ps_df, 
                    asset=asset_name,
                    vix_threshold=vix_threshold,
                    vvix_threshold=vvix_threshold
                )
            elif chosen == "Combine Strategies":
                # For combined strategies, we call our new function.
                return backtest_combined_strategies(
                    ps_df,
                    priorities=priority_strats,
                    asset=asset_name,
                    initial_investment=10_000,
        ...
        # 5) Run backtest for chosen asset(s)
        ...
        if asset_choice == "Both":
            ...
            if "Portfolio_Return" in ymax_res.columns:
            ...
            if "Portfolio_Return" in ymag_res.columns:
            ...
            if "Portfolio_Return" in res.columns:
...
# ==============================================
    ...
    # You can keep or remove the rest as needed...
...
"""
    ...
    st.write("- **Strategy Selection**: Test three strategies with customizable parameters or combine them in a priority order.")
